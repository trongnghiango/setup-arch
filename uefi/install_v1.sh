...
usage() {
    ...
    echo
    echo "An automated Arch Linux installer using rsync for dotfiles deployment."
    echo
    echo "Options:"
    ...
    echo "  -r, --dotfiles-repo <url> URL to your dotfiles repo (e.g., voidrice)."
...
main() {
    ...
    echo "-------------------------------------------------"; echo "Cấu hình sẽ được sử dụng:"; echo "  - User:           ${USER_NAME}"; echo "  - Hostname:       ${HOSTNAME}"; echo "  - Disk:           /dev/${DISK}"; echo "  - Filesystem:     ${FILESYSTEM}"; echo "  - Dotfiles Repo:  ${DOTFILES_REPO}"; echo "-------------------------------------------------"
...
CHROOT_SCRIPT
    ...
    step "Giai đoạn 2: Cài đặt môi trường desktop (Dùng RSYNC)"
...
sudo -u "${USER_NAME_PARAM}" /bin/bash -c '
    ...
    # --- TRIỂN KHAI DOTFILES BẰNG RSYNC (ĐẢM BẢO HOẠT ĐỘNG) ---
    echo "    - Cloning dotfiles repo để sao chép bằng rsync..."
    DOTFILES_DIR="$SRC_DIR/dotfiles_source"
    if ! git clone --depth=1 --recurse-submodules "${DOTFILES_REPO}" "${DOTFILES_DIR}"; then
        echo "!!! LỖI: Không thể clone repo dotfiles tại ${DOTFILES_REPO}." >&2
        exit 1
    fi
    
    echo "    --> DEBUG: Liệt kê nội dung repo đã clone:"
    ls -la "${DOTFILES_DIR}"

    echo "    - Đồng bộ dotfiles vào thư mục HOME..."
    # Thêm cờ --verbose để xem chính xác rsync làm gì
    rsync -av --exclude=".git" --exclude="README.md" --exclude="LICENSE" "${DOTFILES_DIR}/" "$HOME/"

    echo "    --> DEBUG: Liệt kê nội dung thư mục HOME sau khi rsync:"
    ls -la "$HOME"
