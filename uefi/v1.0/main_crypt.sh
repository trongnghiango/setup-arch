...
main() {
    ...
    read -sp "Nhập mật khẩu cho user '${USER_NAME}', 'root' và MÃ HÓA ĐĨA: " PASSWORD; echo; echo # THAY ĐỔI: Chú thích rõ hơn
    ...
    local DEVICE="/dev/${DISK}"; log_info "Phân vùng..." # THAY ĐỔI: Tách log
    ...
    # Sử dụng mật khẩu đã nhập để mã hóa phân vùng LVM một cách tự động
    echo -n "${PASSWORD}" | cryptsetup luksFormat --type luks2 "${PART_LVM}" -
    log_info "Mở khóa phân vùng đã mã hóa..."
    # Mở khóa phân vùng, tạo ra một thiết bị mapper tại /dev/mapper/cryptlvm
    echo -n "${PASSWORD}" | cryptsetup open "${PART_LVM}" cryptlvm -
    ## =========================================================

    log_info "Thiết lập LVM trên thiết bị đã giải mã, Định dạng, Mount..." # THAY ĐỔI: Log gộp
    # ## THAY ĐỔI: Tất cả các thao tác LVM bây giờ sẽ chạy trên /dev/mapper/cryptlvm
    ...
    # (Phần cập nhật keyring không thay đổi)
    ...
    ## MỚI: Lấy UUID của phân vùng LVM để cấu hình GRUB
    local PART_LVM_UUID
...
# THAY ĐỔI: Thêm hook 'encrypt' vào trước 'lvm2'
# Điều này cực kỳ quan trọng để initramfs biết cách giải mã phân vùng trước khi kích hoạt LVM
...
# MỚI: Cấu hình GRUB để nó biết cách tìm và giải mã root filesystem
# Thêm tham số kernel 'cryptdevice'
