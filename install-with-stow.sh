...
usage() {
    ...
    echo
    echo "A fully automated Arch Linux + Desktop installer with intelligent dotfiles management."
    echo
    echo "Options:"
    ...
    echo "  -r, --dotfiles-repo <url> URL to your new, stow-compatible dotfiles repo (optional)."
...
main() {
    ...
    # Repo mới, tương thích với stow. Script sẽ ưu tiên dùng repo này nếu tồn tại.
    local DOTFILES_REPO="https://github.com/trongnghiango/dotfiles.git"
    # Repo cũ (voidrice), dùng làm phương án dự phòng để tự xây dựng cấu trúc stow.
    local VOIDRICE_REPO="https://github.com/trongnghiango/voidrice.git"
    ...
    local TEMP; TEMP=$(getopt -o u:d:H:f:r:h --long user:,disk:,hostname:,filesystem:,dotfiles-repo:,help -n "$0" -- "$@")
    ...
    echo "-------------------------------------------------"; echo "Cấu hình sẽ được sử dụng:"; echo "  - User:           ${USER_NAME}"; echo "  - Hostname:       ${HOSTNAME}"; echo "  - Disk:           /dev/${DISK}"; echo "  - Filesystem:     ${FILESYSTEM}"; echo "  - Stow Repo:      ${DOTFILES_REPO} (ưu tiên)"; echo "  - Fallback Repo:  ${VOIDRICE_REPO} (dự phòng)"; echo "-------------------------------------------------"
...
CHROOT_SCRIPT
    ...
    # GIAI ĐOẠN 2: CÀI ĐẶT DESKTOP VỚI LOGIC REPO KÉP
...
sudo -u "${USER_NAME_PARAM}" /bin/bash -c '
    ...
    DOTFILES_DIR_FOR_STOW="" # Biến này sẽ giữ đường dẫn cuối cùng để stow

    # Kiểm tra xem repo dotfiles (cho stow) có tồn tại không
    if git ls-remote --exit-code "${DOTFILES_REPO}" &>/dev/null; then
        ...
        # Các gói trong .config (trừ zsh, xử lý riêng)
        ...
        # Xử lý ZSH đặc biệt để giải quyết vấn đề ZDOTDIR
        ...
        [ -f "${VOIDRICE_DIR}/.zprofile" ] && mv "${VOIDRICE_DIR}/.zprofile" "${DOTFILES_DIR_FOR_STOW}/zsh/"
        ...
        # Các file ở thư mục HOME khác gom vào các gói logic tương ứng
    ...
    for pkg in "${PACKAGES_TO_STOW[@]}"; do
        if [ -d "$pkg" ]; then
            ...
            echo "        - WARN: Không tìm thấy gói '\''${pkg}'\'' để stow. Bỏ qua."
        fi
    done
...
DESKTOP_SETUP_TEMPLATE
    ...
    sed -i "s|__VOIDRICE_REPO__|${VOIDRICE_REPO}|g" /mnt/root/desktop_setup.sh
